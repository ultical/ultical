...
public interface PlayerMapper extends BaseMapper<Player> {
    ...
    @Select("SELECT id, version, first_name as firstName, last_name as lastName, email, gender, birth_date as birthDate, dfv_number as dfvNumber, is_registered FROM ROSTER_PLAYERS rp, PLAYER p LEFT JOIN DFV_PLAYER ON p.id = DFV_PLAYER.player_id LEFT JOIN UNREGISTERED_PLAYER ON p.id = UNREGISTERED_PLAYER.player_id WHERE p.id = rp.player AND rp.roster = 1")
    List<Player> getByRoster();
}
