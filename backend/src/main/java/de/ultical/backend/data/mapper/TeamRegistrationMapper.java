...
public interface TeamRegistrationMapper extends BaseMapper<TeamRegistration> {
    ...
    final String SELECT_STMT = "SELECT id, version, time_registered, comment, sequence, standing, spirit_score, own_spirit_score, paid, status, not_qualified, roster, team_name, iban";

    // INSERT
    @Insert({ "INSERT INTO TEAM_REGISTRATION",
            "(division_registration, sequence, comment, standing, spirit_score, own_spirit_score, paid, status, not_qualified, roster, team_name, iban)",
            "VALUES (#{divisionRegistrationId}, #{teamReg.sequence, jdbcType=INTEGER}, #{teamReg.comment, jdbcType=VARCHAR}, #{teamReg.standing, jdbcType=INTEGER}, #{teamReg.spiritScore, jdbcType=REAL}, #{teamReg.ownSpiritScore, jdbcType=REAL}, #{teamReg.paid}, #{teamReg.status, jdbcType=VARCHAR}, #{teamReg.notQualified}, #{teamReg.roster.id}, #{teamReg.teamName, jdbcType=VARCHAR}, #{teamReg.iban, jdbcType=VARCHAR})" })
