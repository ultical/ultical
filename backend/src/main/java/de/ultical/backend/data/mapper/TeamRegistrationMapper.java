...
public interface TeamRegistrationMapper extends BaseMapper<TeamRegistration> {
    ...
    final String SELECT_STMT = "SELECT id, version, time_registered, comment, sequence, standing, spirit_score, own_spirit_score, paid, status, not_qualified, roster, team_name, iban, distance, travel_fee";

    // INSERT
    @Insert({ "INSERT INTO TEAM_REGISTRATION",
            "(division_registration, sequence, comment, standing, spirit_score, own_spirit_score, paid, status, not_qualified, roster, team_name, iban, distance, travel_fee)",
            "VALUES (#{divisionRegistrationId}, #{teamReg.sequence, jdbcType=INTEGER}, #{teamReg.comment, jdbcType=VARCHAR}, #{teamReg.standing, jdbcType=INTEGER}, #{teamReg.spiritScore, jdbcType=REAL}, #{teamReg.ownSpiritScore, jdbcType=REAL}, #{teamReg.paid}, #{teamReg.status, jdbcType=VARCHAR}, #{teamReg.notQualified}, #{teamReg.roster.id}, #{teamReg.teamName, jdbcType=VARCHAR}, #{teamReg.iban, jdbcType=VARCHAR}, #{teamReg.distance, jdbcType=INTEGER}, #{teamReg.travelFee, jdbcType=REAL})" })
    ...
    // UPDATE
    // intenionally not updating iban, distance and fee as these values were either computed by the system or are immutable.
    @Override
    ...
    @Results({ @Result(column = "id", property = "id"), @Result(column = "version", property = "version"),
		...
		@Result(column = "team_name", property = "teamName"), @Result(column = "comment", property = "comment"),
		@Result(column = "iban", property = "iban"),
		@Result(column = "distance", property = "property"),
		@Result(column = "travel_fee", property = "travelFee")})
